...
let card_amount = studentCards.CARDS_NO;
let doorState = false;
  ...
  if((readdata[0] == 2)&& (readdata[13] == 3)){
    ...
    if(suckla < 8){
      ...
      if((studentCards[readCardNo].active)){
        ...
        unlockDoor('ACCESS');
        createAccessRecord(a,card.displayName,moment().valueOf())
        break;
      }
      if(!successauth) {
        oledOnDisplay();
...
database.ref('/cards').on('value',function(snapshot) {
...
freevar.uploadNewCard = function (parsedArray){
  ...
  database.ref('/cards/'+Object.keys(tempStudentCards)[0]).set(parsedArray);
}

freevar.changeDoorState = function (newstate){
  database.ref('/userReadable/studentCards/doorState').set(newstate);
}
}
